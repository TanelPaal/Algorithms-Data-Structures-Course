...
public class NodeTest {
   ...
   public void testExceptionMessages() {
      try {
         Node.parsePostfix("((C,D))A");
         fail("Expected RuntimeException");
      } catch (RuntimeException e) {
         assertEquals("Empty node or invalid parentheses usage. Check ((C,D))A", e.getMessage());
      }
   }
   ...
   public void testDeeplyNestedStructure() {
      ...
      Node t = Node.parsePostfix(s);
      String r = t.leftParentheticRepresentation();
      assertEquals("Tree: " + s, "K(J(I(H(G(F(E(D(C(B(A))))))))))", r);
   }
